...
export const deletePost = async (req, res) => {
    try {
        ...
        await User.findByIdAndUpdate(currentUserId, { $inc: { postsCount: -1 } }, { new: true });
...
export const likeOrDislikePost = async (req, res) => {
    try {
        ...
        if (existLike) {
            ...
            const receiverSocketId = await getReceiverSocketId(post.userId);
            if (receiverSocketId) {
...
export const addComment = async (req, res) => {
    try {
        ...
        if (newComment?.user?.profilePic) {
            newComment.user.profilePic = await generateSignedUrl(newComment?.user?.profilePic);
        }
        ...
        if (parentCommentId) {
            if (parentCommentId.toString() !== currentUserId.toString()) {
                ...
                const receiverSocketId = await getReceiverSocketId(parentCommentId);
                if (receiverSocketId) {
    ...
    try {
        ...
        if (existingLike) {
            ...
            if (currentUserId.toString() !== comment.userId.toString()) {
                ...
                const receiverSocketId = await getReceiverSocketId(comment.userId);
                if (receiverSocketId) {
