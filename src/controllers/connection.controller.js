...
export const requestConnection = async (req, res) => {
    try {
        ...
        let userData = await User.findById(fromUserId, {
            _id: 1,
            userName: 1,
            fullName: 1,
            profilePic: 1,
        ...
        let toUserData = await User.findById(toUserId, {
            password: 0,
            email: 0,
            createdAt: 0,
            updatedAt: 0,
        ...
        const receiverSocketId = await getReceiverSocketId(toUserId);
        if (receiverSocketId) {
...
export const rejectConnection = async (req, res) => {
    try {
        ...
        const toUserData = await User.findById(toUserId).select(
            '-password -createdAt -email -updatedAt',
        );
        if (!toUserData) {
            return res.status(400).json({ message: 'User not found.' });
        }
        ...
        const connectionData = await Connection.findOne({ fromUserId, toUserId }, { status: 1 });
...
export const cancelConnection = async (req, res) => {
    try {
        ...
        const userData = await User.findById(fromUserId, {
            password: 0,
            createdAt: 0,
            updatedAt: 0,
            email: 0,
        ...
        if (connectionData.status === 'cancelled') {
            ...
            return res.status(400).json({
                message: 'Your request has been accpted, you can unfollow from thier account.',
            });
        }
...
export const unfollowConnection = async (req, res) => {
    try {
        ...
        await User.findByIdAndUpdate({ _id: fromUserId }, [
            {
                $set: {
                    followingsCount: {
                        $cond: [
