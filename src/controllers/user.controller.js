        ...
        const postData = await Connection.aggregate([
            {
                $match: {
                    $and: [
                        { fromUserId: user?._id },
                        {
                            $or: [{ status: 'accepted' }, { status: 'followed' }],
                        },
                    ],
                },
            },
...
export const fetchUserSavedPosts = async (req, res) => {
    try {
        ...
        let userSavedPosts = await Saved.find({ userId: currentUserId }, { _id: 0, postId: 1 })
            .populate({
                path: 'postId',
                select: 'media',
            })
...
export const updateAbout = async (req, res) => {
    try {
        ...
        const updatedUser = await User.findByIdAndUpdate(currentUserId, { about }, { new: true });

        if (updatedUser) {
            return res
                .status(200)
                .json({ success: true, message: 'about updated', about: updatedUser.about });
        } else {
