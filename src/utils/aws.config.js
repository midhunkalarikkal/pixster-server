...
if (hasAwsConfig) {
    ...
    console.warn(
        '[aws.config] Missing AWS configuration; S3 features will be disabled until env vars are set.',
    );
}
...
export async function generateSignedUrl(key, expires = 172800) {
    if (!s3Client) {
        throw new Error(
            'AWS S3 is not configured. Set AWS_REGION, AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, and AWS_S3_BUCKET_NAME.',
        );
    }
